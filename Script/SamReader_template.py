...
def Store(file,minval):
    if check_file(file): #We check if the file is not empty and exists
        Qual=ReadQuality(SamRead2(file, -1)) #The minval=-1 signifies here that we want to compute the quality for each value of quality (not from the minval and up)
        sam_lines=SamRead(file,minval) #We obtain a list of lines 
        unmapped_count,unmappedDico=unmapped(sam_lines) #We obtain the number of unmapped reads and a dictionary containing them
        partially_mapped_count,partiallyMappedDico=partiallyMapped(sam_lines) #We obtain the number of partiallyMapped reads and a dictionary containing them
        PerfectlyMappedDico=PerfectMapped(sam_lines) #We obtain a dictionary containing the IDs of perfectly mapped reads (we keep only one occurrence per pair since we are not interested in the line content)
        ReadsId=Ids(file,minval) # We build a dictionary of read IDs with their occurrence counts (to verify that each read is properly paired)
        return unmapped_count,partially_mapped_count,PerfectlyMappedDico,unmappedDico,partiallyMappedDico,ReadsId,Qual
    else:
        return None #Change for an exit
    
def Analyse(PerfectlyMappedDico,unmappedDico,partiallyMappedDico,ReadsId,file,minval): # Function that analyzes read pairs and calculates the global percentage of each mutation based on the CIGAR string
    ...
    for s in ReadsId.keys(): #We iterate over each read ID
        if s in PerfectlyMappedDico and s in unmappedDico: #We check if the pair is present in perfectMapped and Unmapped
            MapUnMap[s]=unmappedDico[s] #We add the first read of the pair
            MapUnMap[s+"-1"]=PerfectlyMappedDico[s] #We add the second read of the pair
        if s in PerfectlyMappedDico and s in partiallyMappedDico:  #We check if the pair is present in perfectMapped and partiallyMapped
            MapParMap[s]=partiallyMappedDico[s] #We add the first read of the pair
            MapParMap[s+"-1"]=PerfectlyMappedDico[s] #We add the second read of the pair
        with open("../Results/outpuTable_cigar.txt","a+") as outpuTable: #We will write the mutation percentages for each line
            if ReadsId[s]==2: #Verify that the ID is associated with a read pair
...
def ReadQuality(sam_line): #Function that computes the number of reads per value of quality, the results are stored in a .tsv file 
    QualityDico={} 
    for line in sam_line.values(): #We iterate through each line
        value=int(line[3]) #Save the value of the quality
        if value not in QualityDico.keys():
            QualityDico[value]=1 #We associate each value of quality to a count (of reads)
        else :
            QualityDico[value]+=1 
    with open("../Results/QualityDistribution.tsv","a+") as outpuTable: #We write the results
        outpuTable.write("Quality"+",\t"+"Number \n")
        for k in QualityDico.keys():
            outpuTable.write(str(k)+"\t"+str(QualityDico[k])+"\n")
    return QualityDico
            

#Converts flags into binary
def flagBinary(flag):
...
def unmapped(sam_line):
    unmappedDico={} #Dictionnary that will contains unmapped reads
    unmapped_count = 0
...
def partiallyMapped(sam_line):
    ...
    with open ("../Results/only_partially_mapped.fasta", "a+") as partillay_mapped_fasta, open("../Results/summary_partially_mapped.txt", "w") as summary_file:
        for line in sam_line:
            ...
            if int(flag[-2]) == 1: 
                if col_line[5] != str(len(col_line[9]))+"M": #à spécifier 
                    partiallyMappedDico[col_line[0]]=col_line[1:]
        ...
        return partially_mapped_count,partiallyMappedDico

def PerfectMapped(sam_line):
    PerfectlyMappedDico={} #Dictionnary that will contains perfectly mapped reads
    for line in sam_line:
        col_line = line.split("\t")
        if int(flagBinary(col_line[1])[-3])==0 and col_line[5] == str(len(col_line[9]))+"M":
            PerfectlyMappedDico[col_line[0]]=col_line[1:]
    return PerfectlyMappedDico

def isPartMapped(col_line): #function that checks if a read is partiallyMapped
    return int(flagBinary(col_line[1])[-2]) ==1 and col_line[5] != "100M"

def isunmapped(col_line): #function that checks if a read is unmapped
    return int(flagBinary(col_line[1])[-3]) == 1

def readCigar(cigar): 
    ext = re.findall(r'\w',cigar) # Split all characters
    key=[] 
    value=[]    
    val=""
...
def SamRead2(file,minval): # Function that stores reads in a dictionary with Key = ID and Value = the rest of the line. Also takes into account the minimal value, like SamRead
    ReadDico={}
    with open(file,"r") as Str:  
        L=list(islice(enumerate(Str),2,None))
    List=[t[1] for t in L if int(t[1].split("\t")[4]) > minval]
